  ...
  try {
    ...
    if (task === 'fullpage' || task === 'tweak') {
      const sysFP = [
        ...
        'https://image.pollinations.ai/prompt/<url-encoded scene>?width=1200&height=1500&nologo=true&nofeed=true&enhance=true&model=flux — and write a RICH, specific, photographic scene (subject + setting + golden-hour lighting + 35mm/50mm lens + shallow depth of field + "editorial magazine-quality photograph, hyper-detailed" + "no text no watermark"). Descriptive prompts = far better images.',
        'IMAGE AUTHENTICITY (applies to EVERY image, EVERY industry): each scene must depict the ACTUAL commercial premises and working reality of this specific business type — its characteristic equipment, fixtures, tools, and environment, instantly recognizable as that kind of establishment (a tire shop shows lifts/tire racks/a service bay; a dentist shows an operatory chair and equipment; a yoga studio shows mats, mirrors, a props shelf; a restaurant shows the kitchen line or dining room). NEVER a villa, a mansion, a generic pretty interior, or an idealized set that could belong to any business. For testimonial avatars use a real portrait scene ("candid editorial portrait of a warm smiling <persona>, natural light, shallow depth of field, no text").',
        '',
        ...
        '7. Testimonials: if a REAL rating/review count is provided, show it prominently (e.g. 5.0 on Google with 7 reviews). Quote cards MUST be clearly labeled sample content: add a small caption Sample review - replace with your real customer reviews before launch under each quote, and use first-name-only placeholders. NEVER invent named customer reviews presented as real ones (the FTC fake-review rule, 16 CFR Part 465, bans fabricated testimonials on live business sites).',
      ...
      const TWEAK_SYS = 'You are a meticulous web developer editing an EXISTING finished page. Apply ONLY the requested change with the SMALLEST reasonable interpretation. Keep every other element, style, image, headline, and line of the document IDENTICAL — never redesign, rename, reword, recolor, or "improve" anything that was not explicitly asked for. Never alter headlines or body copy unless the request explicitly asks for that text to change. If the request includes an image URL, insert it as an <img> tag where indicated. Return ONLY the complete updated HTML document (<!DOCTYPE html> … </html>), no commentary.';
